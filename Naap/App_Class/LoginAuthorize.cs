...
public class LoginAuthorize : AuthorizeAttribute
{
    ...
    protected override bool AuthorizeCore(HttpContextBase httpContext)
    {
        ...
        if (lists.Count > 0)
        {
            ...
            {
                if (MemberAccount.RoleName == role) { bln_authorized = true; break; }
            }
        }
    ...
    {
        //預設為 IIS 位址,加入虛擬目錄名稱 orderfood02
        //string str_route = "/orderfood02/User/Login";
        string str_route = "/Naap/Member/Login";
        string str_host = HttpContext.Current.Request.Url.Host.ToLower();
        //如果為本機(localhost)測試,不用加虛擬目錄名稱 Naap
        if (str_host.Contains("localhost")) str_route = "/Member/Login";
        //返回登入頁
